...
def play_alert_sound():
    """Play alert sound for high traffic"""
...
def settings_panel():
    """Enhanced settings panel with UI controls"""
    st.markdown("""
    ...
    with col2:
        if st.button("🔍 Fullscreen" if not st.session_state.fullscreen_mode else "🔍 Exit Fullscreen"):
            st.session_state.fullscreen_mode = not st.session_state.fullscreen_mode
            st.rerun()

    with col3:
        if st.button("🔊 Sound ON" if st.session_state.sound_alerts else "🔇 Sound OFF"):
            st.session_state.sound_alerts = not st.session_state.sound_alerts
            st.rerun()
...
def run_dashboard():
    ...
    fullscreen_class = "fullscreen-mode" if st.session_state.fullscreen_mode else ""

    st.markdown(f'<div class="{theme_class} {fullscreen_class}">', unsafe_allow_html=True)
    ...
    while True:
        ...
        # Sound alerts for high traffic
        if st.session_state.sound_alerts and total_vehicles > 30 and not alert_triggered:
            play_alert_sound()
        ...
        if current != last_summary:
            ...
            <div class="summary-box">
                ...
                <div class="summary-content">
                    ...
                    <strong>🚨 Total Vehicles:</strong> {total_vehicles} 
                    {"<span class='alert-text'>⚠️ HIGH TRAFFIC ALERT!</span>" if total_vehicles > 30 else ""}
                </div>
            </div>
            """, unsafe_allow_html=True)
